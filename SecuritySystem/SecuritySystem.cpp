...
bool isPersonAllowed (std::string person)
{
	...
	for (int x = 0; x < (sizeof (ALLOW_ACCESS) / sizeof (char*)); x++)
	{
		...
		{
			is_allowed = true;  // original: is_allowed == true;
			break;
		}
	}
...
void ssDoorLockEventHandler (DoorLock::DoorLockEvent event, string pin_code)  // original: void ssDoorLockEventHandler (DoorLock::DoorLockEvent event, int pin_code)
{
	...
	switch (event)
	{
		...
		{
			if (allowed_person_inside)  // original: if allowed_person_inside)
			{
...
void ssVideoDoorbellEventHandler (VideoDoorbell::VideoDoorbellEvent event, std::string person_at_door)
{
	...
	switch (event)
	{
		...
		case VideoDoorbell::VideoDoorbellEvent::SOMEONE_AT_THE_DOOR:
		{
			...
			if (isPersonAllowed (person_at_door))  // original: if (!isPersonAllowed (person_at_door))
			{
				access_granted = true;
				DoorLock::unlockDoor ();
			}
		...
		{
			MobileAppInterface::notifyFrontDoorActivity(false);  // original: MobileAppInterface::notifyFrontDoorActivity (false)
		} break;
	}
}
